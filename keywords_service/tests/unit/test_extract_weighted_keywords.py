from keywords_service.domain import weighted_keywords
...
def test_user_config():
	...
	result = weighted_keywords.extract("x r z v", 
		user_config=user_config)
	assert list(result.keys()) == ["x", "r", "z", "v"]
	assert all([v.weight == 0.25 for v in result.values()])

def test_apply_threshold():
	result = weighted_keywords.apply_threshold(test_weights, goal=0.05)
	assert(len(result) == 1)
	assert("keyword1" in result.keys())

	result = weighted_keywords.apply_threshold(test_weights, goal=0.07)
	assert(len(result) == 2)
	assert("keyword1" in result.keys())
	assert("keyword2" in result.keys())

	result = weighted_keywords.apply_threshold(test_weights, goal=1.0)
	assert(len(result) == len(test_weights))
	assert result.keys() == test_weights.keys()

	result = weighted_keywords.apply_threshold(test_weights, goal=0)
	assert(len(result) == 0)

def test_calculate_weights():
	counts = {"keyword1" : 5, "keyword2": 3, "keyword3": 1}
	total = sum(counts.values())
	result = weighted_keywords.calculate_weights(counts, total)
...
def test_get_ngrams():
	...
	result = weighted_keywords.get_ngrams(tokens, max_n=2)
	assert len(result) == 2
	assert "keyword1_keyword2" in result
	assert "keyword2_keyword3" in result

	result = weighted_keywords.get_ngrams(tokens, max_n=3)
	assert len(result) == 3
	assert "keyword1_keyword2" in result
	assert "keyword2_keyword3" in result
...
def test_lemmatize():
	...
	result = list(weighted_keywords.lemmatize(words))
	assert len(result) == 3
...
def test_length_threshold():
	...
	result = list(weighted_keywords.length_threshold(words, threshold=3))

	assert len(result) == 3
	assert "a" not in result
	assert "ab" not in result

	result = list(weighted_keywords.length_threshold(words, threshold=10))
...
def test_strip_punct():
	...
	result = weighted_keywords.strip_punct(string)
	assert result == "a punctuated string"

def test_stopwords():
	string = "the"
	result = weighted_keywords.not_stopword(string)
	assert not result
...
def test_cleanup_text():
	...
	result = list(weighted_keywords.cleanup_text(string, threshold=3))
